...
template <typename T> class Rect
{
  public:
	...
	constexpr Rect(T x0 = 0, T y0 = 0, T x1 = 0, T y1 = 0) : Rect(Vec2<T>(x0, y0), Vec2<T>(x1, y1))
	{
	}
	constexpr Rect(Vec2<T> p0, Vec2<T> p1) : p0(p0), p1(p1) {}

	Vec2<T> p0, p1;

	[[nodiscard]] constexpr bool within(Vec2<T> p) const
	{
		return (p.x >= p0.x && p.x < p1.x && p.y >= p0.y && p.y < p1.y);
	}
	[[nodiscard]] constexpr bool withinInclusive(Vec2<T> p) const
	{
		return (p.x >= p0.x && p.x <= p1.x && p.y >= p0.y && p.y <= p1.y);
	}
	[[nodiscard]] constexpr bool within(Rect<T> r) const
	{
		return (r.p0.x >= p0.x && r.p1.x <= p1.x && r.p0.y >= p0.y && r.p1.y <= p1.y);
	}
	[[nodiscard]] constexpr bool intersects(Rect<T> r) const
	{
		return !(r.p1.x <= p0.x || r.p0.x >= p1.x || r.p1.y <= p0.y || r.p0.y >= p1.y);
	}

	[[nodiscard]] constexpr T getWidth() const { return (this->p1.x - this->p0.x); }

	[[nodiscard]] constexpr T getHeight() const { return (this->p1.y - this->p0.y); }
	...
	[[nodiscard]] constexpr Vec2<T> size() const { return Vec2<T>{p1.x - p0.x, p1.y - p0.y}; }
	...
	// Attempts to merge adjacent rects in a set, returns the number merged.
	// Keeps scanning until a full pass merges nothing.
		...
		do
		{
			...
			for (auto it1 = rectSet.begin(); it1 != rectSet.end();)
			{
				...
				for (auto it2 = rectSet.begin(); it2 != rectSet.end();)
				{
					...
					//	+----+----+
					//
					//	No need to check other way as r1 & r2 will be compared in both orders in
					//	the loop
					//
